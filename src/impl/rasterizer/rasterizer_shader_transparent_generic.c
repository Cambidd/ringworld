...
ShaderTransparentGenericInstance *rasterizer_shader_transparent_generic_get_or_create_instance(ShaderTransparentGeneric *tag) {
    ...
    if(shader_transparent_generic_instances->count == MAX_SHADER_TRANSPARENT_GENERIC_INSTANCES) {
        // Just nuking the instance table if it fills up *should* be safe as tag count is always >= required instances 
        // for a given map. In theory it should already have crashed if we can't actually fit them all.
        ...
        for(size_t map_index = 0; map_index < 4; map_index++) {
            if(map_index < shader_data->maps.count) {
                ...
                if(*af_is_enabled && (d3d9_device_caps.RasterCaps & D3DPRASTERCAPS_ANISOTROPY) != 0 && 1 < d3d9_device_caps.MaxAnisotropy) {
                    uint32_t max_anisotropy = d3d9_device_caps.MaxAnisotropy < 16 ? d3d9_device_caps.MaxAnisotropy : 16;
                    rasterizer_dx9_set_sampler_state(map_index, D3DSAMP_MAXANISOTROPY, max_anisotropy);
